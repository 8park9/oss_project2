...
#https://github.com/8park9/oss_project2
...
def load_dataset(dataset_path):
	frame = DataFrame(pd.read_csv(dataset_path))
	return frame


def dataset_stat(dataset_df):
	n_feats = len(dataset_df.drop(columns="target", axis=1).columns)
	n_class0 = len(dataset_df['target'][dataset_df.target == 0])
	n_class1 = len(dataset_df['target'][dataset_df.target == 1])
